...
const eras = {
    ...
    bone_age: {
        ...
        buildings: ['house', 'reproduction_house', 'warrior_camp', 'bonfire', 'farm', 'forester_hut']
    }
};
...
function drawBuildingPanel() {
    ...
    // Кнопка хижины лесника (только в новокаменном веке)
    if (canBuildInCurrentEra('forester_hut')) {
        const foresterHutButtonX = isMobile ? 570 : 720; // Адаптивная позиция X
        const foresterHutButtonY = panelY + (isMobile ? 30 : 40);
        
        // Проверяем, достаточно ли ресурсов для хижины лесника (требуется 10 дерева и 5 камня)
        const canBuildForesterHut = resources.wood >= 10 && resources.stone >= 5;
        
        // Фон кнопки
        ctx.fillStyle = canBuildForesterHut ? (buildingMode && buildingType === 'forester_hut' ? '#27ae60' : '#34495e') : '#7f8c8d';
        ctx.fillRect(foresterHutButtonX, foresterHutButtonY, buttonWidth, buttonHeight);
        
        // Рамка кнопки
        ctx.strokeStyle = canBuildForesterHut ? '#fff' : '#95a5a6';
        ctx.lineWidth = 2;
        ctx.strokeRect(foresterHutButtonX, foresterHutButtonY, buttonWidth, buttonHeight);
        
        // Текст кнопки
        ctx.font = '14px Arial';
        ctx.fillStyle = canBuildForesterHut ? '#fff' : '#95a5a6';
        ctx.textAlign = 'center';
        ctx.fillText('🌲 Хижина', foresterHutButtonX + buttonWidth/2, foresterHutButtonY + 18);
        ctx.fillText('лесника', foresterHutButtonX + buttonWidth/2, foresterHutButtonY + 32);
        
        // Стоимость
        ctx.font = '12px Arial';
        ctx.fillStyle = canBuildForesterHut ? '#bdc3c7' : '#7f8c8d';
        ctx.fillText('10 дерева, 5 камня', foresterHutButtonX + buttonWidth/2, foresterHutButtonY + buttonHeight + 15);
        
        // Сохраняем координаты кнопки хижины лесника для обработки кликов
        window.foresterHutButtonBounds = {
            x: foresterHutButtonX,
            y: foresterHutButtonY,
            width: buttonWidth,
            height: buttonHeight,
            canBuild: canBuildForesterHut
        };
    }
...
function drawBuildingMessage() {
    ...
    switch(buildingType) {
        ...
        case 'farm':
            ...
            break;
        case 'forester_hut':
            message = window.innerWidth <= 800 ? 'Тапните для постройки хижины лесника' : 'Кликните на карту, чтобы построить хижину лесника (производство дерева)';
            color = '#8B4513';
            break;
    }
...
function drawBuilding(building) {
    ...
    if (building.type === 'house') {
        ...
        // Показываем здоровье если повреждено
        if (building.health && building.health < building.maxHealth) {
            ...
            const healthWidth = barWidth * healthPercent;
            ctx.fillRect(-barWidth/2, -50, healthWidth, barHeight);
            
            // Рамка полосы здоровья
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 1;
            ctx.strokeRect(-barWidth/2, -50, barWidth, barHeight);
        }
    } else if (building.type === 'forester_hut') {
        // Хижина лесника - деревообрабатывающее здание
        
        // Основное здание - деревянная хижина
        ctx.fillStyle = '#8B4513';
        ...
        ctx.fillRect(-30, 25, 12, 12);
        ctx.fillRect(18, 25, 12, 12);
        ctx.strokeStyle = '#654321';
        ctx.lineWidth = 1;
        ctx.strokeRect(-30, 25, 12, 12);
        ctx.strokeRect(18, 25, 12, 12);
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(18, -45, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(22, -50, 2, 0, Math.PI * 2);
        ctx.fill();
        
        // Поленница рядом с хижиной
        ctx.fillStyle = '#8B4513';
        for (let i = 0; i < 8; i++) {
            ctx.fillRect(-60 + i * 3, 45, 2, 15);
        }
        ctx.strokeStyle = '#654321';
        ctx.lineWidth = 1;
        ctx.strokeRect(-60, 45, 24, 15);
        ...
        // Показываем производство дерева
        ctx.font = '14px Arial';
        ctx.fillStyle = '#27ae60';
        ctx.textAlign = 'center';
        ctx.fillText('🌲', 0, -40);
        ...
        if (building.health < building.maxHealth) {
            ...
            // Фон полосы здоровья
            ctx.fillStyle = 'rgba(255, 0, 0, 0.7)';
            ctx.fillRect(-barWidth/2, -55, barWidth, barHeight);
            
            // Полоса здоровья
            ctx.fillStyle = healthPercent > 0.5 ? 'rgba(0, 255, 0, 0.8)' : 
                           healthPercent > 0.25 ? 'rgba(255, 255, 0, 0.8)' : 'rgba(255, 0, 0, 0.8)';
            const healthWidth = barWidth * healthPercent;
            ctx.fillRect(-barWidth/2, -55, healthWidth, barHeight);
            
            // Рамка полосы здоровья
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 1;
            ctx.strokeRect(-barWidth/2, -55, barWidth, barHeight);
        }
    }
...
function drawCaveWorld() {
    ...
    // Консоль разработчика поверх всего
    drawDevConsole();
}
...
function updateFarms() {
    buildings.forEach(building => {
        if (building.type === 'farm') {
            ...
            // Логика хижины лесника - производит дерево каждые 10 секунд
            if (!building.lastProduction) {
                building.lastProduction = currentTime;
            }
            
            if (currentTime - building.lastProduction > 10000) { // 10 секунд
                resources.wood += 1; // Производит 1 дерево
                totalWoodCollected += 1;
                building.lastProduction = currentTime;
                
                // Уведомление о производстве
                console.log('Хижина лесника произвела 1 дерево');
            }
...
canvas.addEventListener('mousedown', function(e) {
    ...
    // Проверка клика по кнопке консоли разработчика (работает всегда)
    if (window.devConsoleButtonBounds) {
        const btn = window.devConsoleButtonBounds;
        if (screenX >= btn.x && screenX <= btn.x + btn.width && 
            screenY >= btn.y && screenY <= btn.y + btn.height) {
            devConsoleOpen = !devConsoleOpen;
            devConsoleInput = '';
            console.log('Dev console toggled:', devConsoleOpen);
            return;
        }
    }
    ...
    if (hasBuilders) {
        ...
        // Кнопка хижины лесника
        if (window.foresterHutButtonBounds) {
            const btn = window.foresterHutButtonBounds;
            if (screenX >= btn.x && screenX <= btn.x + btn.width && 
                screenY >= btn.y && screenY <= btn.y + btn.height && btn.canBuild) {
                // Активируем режим строительства хижины лесника
                buildingMode = true;
    ...
    if (buildingMode && buildingType && hasBuilders) {
        ...
        } else if (buildingType === 'forester_hut' && resources.wood >= 10 && resources.stone >= 5) {
            // Проверяем, не слишком ли близко к другим зданиям
            let canBuild = true;
            buildings.forEach(building => {
                const distance = Math.sqrt((mx - building.x) ** 2 + (my - building.y) ** 2);
...
function updateSabertoothTigers() {
    ...
    sabertoothTigers.forEach(tiger => {
        ...
        if (tiger.target && tiger.target.health > 0) {
            ...
            // Если нет цели среди людей, ищем здания для атаки (фермы и хижины лесника)
            let closestBuilding = null;
            let closestBuildingDistance = Infinity;
            
            buildings.forEach(building => {
                if (building.type === 'farm' || building.type === 'forester_hut') {
                    const distance = Math.sqrt((tiger.x - building.x) ** 2 + (tiger.y - building.y) ** 2);
                    if (distance < tiger.detectionRange && distance < closestBuildingDistance) {
            ...
            if (closestBuilding) {
                // Движение к зданию
                const dx = closestBuilding.x - tiger.x;
                const dy = closestBuilding.y - tiger.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                ...
                if (distance > tiger.attackRange) {
                    ...
                    // Атака здания
                    if (currentTime - tiger.lastAttack > 2000) { // Атака каждые 2 секунды
                        tiger.lastAttack = currentTime;
                        
                        // Нанести урон зданию
                        if (closestBuilding.health) {
                            closestBuilding.health -= tiger.damage;
                            console.log(`Тигр атакует ${closestBuilding.type}! Здоровье: ${closestBuilding.health}/${closestBuilding.maxHealth}`);
                            
                            if (closestBuilding.health <= 0) {
                                // Уничтожить здание
                                const buildingIndex = buildings.indexOf(closestBuilding);
                                if (buildingIndex > -1) {
                                    buildings.splice(buildingIndex, 1);
                                    console.log(`${closestBuilding.type} разрушено тигром!`);
                                }
                            }
                        }
                    }
...
function updateMammoths() {
    ...
    mammoths.forEach(mammoth => {
        ...
        // Если людей нет рядом - проверяем наличие зданий для атаки
        else {
            // Сначала ищем здания для атаки (фермы и хижины лесника)
            let closestBuilding = null;
            let closestBuildingDistance = Infinity;
            
            buildings.forEach(building => {
                if (building.type === 'farm' || building.type === 'forester_hut') {
                    const distance = Math.sqrt((mammoth.x - building.x) ** 2 + (mammoth.y - building.y) ** 2);
                    if (distance < 200 && distance < closestBuildingDistance) { // Мамонты чуют здания на расстоянии 200
                        closestBuilding = building;
                        closestBuildingDistance = distance;
                    }
                }
            ...
            if (closestBuilding && closestBuildingDistance < 200) {
                mammoth.angry = true;
                mammoth.fleeing = false;
                
                // Движение к зданию
                const dx = closestBuilding.x - mammoth.x;
                const dy = closestBuilding.y - mammoth.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                
                if (distance > 60) { // Радиус атаки мамонта
                    // Движение к зданию
                    const moveX = (dx / distance) * mammoth.speed;
                    const moveY = (dy / distance) * mammoth.speed;
                    
                    mammoth.x += moveX;
                    mammoth.y += moveY;
                } else {
                    // Атака здания
                    if (currentTime - (mammoth.lastBuildingAttack || 0) > 3000) { // Атака каждые 3 секунды
                        mammoth.lastBuildingAttack = currentTime;
                        
                        // Нанести урон зданию
                        if (closestBuilding.health) {
                            closestBuilding.health -= mammoth.damage || 40; // Урон мамонта по зданиям
                            console.log(`Мамонт атакует ${closestBuilding.type}! Здоровье: ${closestBuilding.health}/${closestBuilding.maxHealth}`);
                            ...
                            if (closestBuilding.health <= 0) {
                                ...
                                if (buildingIndex > -1) {
                                    buildings.splice(buildingIndex, 1);
                                    console.log(`${closestBuilding.type} разрушено мамонтом!`);
                                }
                            }
                        }
                    }
                ...
                // Нет зданий рядом - мирно пасется
                mammoth.angry = false;
                mammoth.fleeing = false;
...
function drawPauseMenu() {
    ...
    // Кнопка и окно консоли разработчика
    drawDevConsoleButton();
    drawDevConsole();
}
...
// Консоль разработчика
function drawDevConsoleButton() {
    const buttonSize = 40;
    const buttonX = canvas.width - buttonSize - 10;
    const buttonY = 10;
    
    // Сохраняем границы кнопки для обработки кликов
    window.devConsoleButtonBounds = {
        x: buttonX,
        y: buttonY,
        width: buttonSize,
        height: buttonSize
    ...
    // Фон кнопки
    ctx.save();
    ctx.fillStyle = devConsoleOpen ? 'rgba(0, 255, 0, 0.8)' : 'rgba(0, 0, 0, 0.7)';
    ...
    // Иконка консоли (символ >_)
    ctx.fillStyle = '#00ff00';
    ctx.font = 'bold 20px monospace';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('>_', buttonX + buttonSize/2, buttonY + buttonSize/2);
    
    ctx.restore();
}

function executeDevConsoleCommand(command) {
    command = command.trim();
    if (!command) return;
    
    // Добавляем в историю
    devConsoleHistory.push(command);
    if (devConsoleHistory.length > devConsoleMaxHistory) {
        devConsoleHistory.shift();
    }
    ...
    if (cmd === '/age') {
        const ageNum = parseInt(parts[1]);
        if (ageNum === 1) {
            currentEra = 'stone_age';
            eras.stone_age.unlocked = true;
            devConsoleHistory.push('> Переключено на Каменный век');
            console.log('Dev Console: Switched to Stone Age');
        } else if (ageNum === 2) {
            currentEra = 'bone_age';
            eras.bone_age.unlocked = true;
            devConsoleHistory.push('> Переключено на Новокаменный век');
            console.log('Dev Console: Switched to Bone Age');
        } else {
            devConsoleHistory.push('> Ошибка: используйте /age 1 или /age 2');
        }
    } else {
        devConsoleHistory.push('> Неизвестная команда: ' + command);
    }
...
function drawDevConsole() {
    ...
    // Окно консоли
    const consoleWidth = Math.min(600, canvas.width - 40);
    const consoleHeight = 400;
    const consoleX = (canvas.width - consoleWidth) / 2;
    const consoleY = (canvas.height - consoleHeight) / 2;
    ...
    ctx.fillStyle = '#00ff00';
    ctx.font = 'bold 20px monospace';
    ctx.fillText('Developer Console', consoleX + 10, consoleY + 30);
    
    // Подсказка
    ctx.font = '14px monospace';
    ctx.fillStyle = '#888';
    ctx.fillText('Commands: /age 1 (Stone Age), /age 2 (Bone Age)', consoleX + 10, consoleY + 55);
    ctx.fillText('Press ESC to close, ENTER to execute', consoleX + 10, consoleY + 75);
    
    // История команд
    ctx.fillStyle = '#00ff00';
    const historyStartY = consoleY + 100;
    const displayHistory = devConsoleHistory.slice(-8); // Показываем последние 8 команд
    displayHistory.forEach((line, index) => {
        ctx.fillText(line, consoleX + 10, historyStartY + index * 20);
    });
    
    // Поле ввода
    const inputY = consoleY + consoleHeight - 40;
    ctx.fillStyle = '#2a2a2a';
    ctx.fillRect(consoleX + 10, inputY, consoleWidth - 20, 30);
    
    ctx.strokeStyle = '#00ff00';
    ctx.lineWidth = 1;
    ctx.strokeRect(consoleX + 10, inputY, consoleWidth - 20, 30);
    
    // Текст ввода
    ctx.fillStyle = '#00ff00';
    ctx.font = '16px monospace';
    ctx.fillText('> ' + devConsoleInput + '_', consoleX + 15, inputY + 20);
...
document.addEventListener('keydown', function(e) {
    ...
    // Консоль разработчика по клавише T (работает всегда, даже в меню)
    if ((e.key === 't' || e.key === 'T' || e.key === 'е' || e.key === 'Е') && !devConsoleOpen && e.ctrlKey === false && e.altKey === false) {
        e.preventDefault();
        devConsoleOpen = true;
        devConsoleInput = '';
    ...
    // Если консоль открыта, обрабатываем ввод
    if (devConsoleOpen) {
        e.preventDefault(); // Блокируем все клавиши когда консоль открыта
        
        if (e.key === 'Escape') {
            devConsoleOpen = false;
            devConsoleInput = '';
            console.log('Dev console closed');
            return;
        }
        
        if (e.key === 'Enter') {
            executeDevConsoleCommand(devConsoleInput);
            devConsoleInput = '';
            return;
        }
        
        if (e.key === 'Backspace') {
            devConsoleInput = devConsoleInput.slice(0, -1);
            return;
        }
        
        // Добавляем символ в ввод
        if (e.key.length === 1) {
            devConsoleInput += e.key;
            return;
        }
        
        return; // Блокируем все остальные клавиши
    }
...
function handleTouchStart(e) {
    ...
    // Проверяем клик по кнопке консоли (работает всегда)
    ...
    if (window.devConsoleButtonBounds) {
        ...
        if (touchX >= btn.x && touchX <= btn.x + btn.width && 
            ...
            devConsoleOpen = !devConsoleOpen;
            devConsoleInput = '';
            console.log('Dev console toggled (touch):', devConsoleOpen);
            return;
        }
    }
    ...
    if (buildingMode && buildingType && hasBuilders) {
        if (buildingType === 'house' && resources.wood >= 10) {
            ...
            if (canBuild) {
                ...
                buildingMode = false;
                buildingType = null;
                console.log('Touch - Построена хижина лесника');
                return;
            }
        }
